...
public class IndentsVisitor<P> extends YamlIsoVisitor<P> {
    ...
    public @Nullable Yaml preVisit(Yaml tree, P p) {
        ...
        if (y instanceof Yaml.Scalar && StringUtils.hasLineBreak(((Yaml.Scalar) y).getValue())) {
            String newValue = ((Yaml.Scalar) y).getValue().replaceAll("\\R", lineSeparator() + repeat(" ", indent));
    ...
    private String indentComments(String prefix, int column) {
        ...
        if (prefix.contains("#")) {
            ...
            while (m.find()) {
                ...
                int newlineCount = StringUtils.countOccurrences(whitespace, lineSeparator());
                if (firstLine && newlineCount == 0) {
                    ...
                    result.append(repeat(lineSeparator(), newlineCount))
                            .append(indent)
                            .append(comment);
                }
                firstLine = false;
